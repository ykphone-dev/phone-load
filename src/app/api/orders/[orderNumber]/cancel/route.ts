...
import { cancelBeforeDeposit, requestCancel } from "@/features/used-phones/server/orders";
...
export async function POST(request: NextRequest, { params }: { params: { orderNumber: string } }) {
  return handle(async () => {
    const body = await readJsonOptional<{ reason?: string; afterDeposit?: boolean }>(request);
