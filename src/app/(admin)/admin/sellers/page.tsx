...
const STATUSES: SelectSeller["status"][] = [
  "PENDING",
  "APPROVED",
  "REJECTED",
  "SUSPENDED",
...
export default async function AdminSellersPage({ searchParams }: Props) {
  ...
  const status = STATUSES.includes(raw as SelectSeller["status"])
    ? (raw as SelectSeller["status"])
    : undefined;
  ...
  return (
    <AdminShell
      heading="판매자 관리"
      description="입점 신청 승인·반려, 판매자 정지 및 해제를 처리합니다."
    >
      <StatusFilterLinks
      ...
      {sellers.length === 0 ? (
        <EmptyState
          title="판매자가 없습니다."
          description="조건에 해당하는 판매자가 없습니다."
        />
      ) : (
          ...
          <Table>
            ...
            <TableBody>
              {sellers.map((s) => (
                <TableRow
                  key={s.id}
                  className={s.status === "PENDING" ? "bg-amber-50" : undefined}
                >
