  ...
  if (!url) {
    return (
      <span className="text-zinc-400">
        {label.replace(" 보기", "")} 미제출
      </span>
    );
  }
  return (
    <a href={url} target="_blank" rel="noreferrer" className={cn(buttonVariants({ variant: "outline", size: "sm" }))}>
      {label}
    </a>
  );
}
...
export default async function AdminSellerDetailPage({ params }: Props) {
  ...
  const { seller, businessLicenseSignedUrl, mailOrderLicenseSignedUrl, products } = data;
    ...
    >
      <section className="mb-6 flex flex-wrap items-center justify-between gap-3 border p-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-zinc-500">
            ...
            {seller.approvedAt && ` · 승인일 ${formatDateTime(seller.approvedAt)}`}
          </span>
        </div>
      ...
      {seller.rejectReason && (
        <Alert variant="destructive" className="mb-6">
          <AlertTitle>{seller.status === "SUSPENDED" ? "정지 사유" : "반려 사유"}</AlertTitle>
          <AlertDescription className="whitespace-pre-wrap">{seller.rejectReason}</AlertDescription>
        </Alert>
      )}
        ...
        <section>
          ...
          <KeyValueList
            items={[
              ...
              { label: "사업자등록번호", value: formatBusinessNumber(seller.businessNumber) },
              { label: "통신판매업 신고번호", value: seller.mailOrderNumber },
              {
                label: "사업장 주소",
                value: [seller.address, seller.addressDetail].filter(Boolean).join(" "),
              },
              { label: "담당자", value: seller.contactName },
              { label: "담당자 휴대폰", value: seller.contactPhone },
              { label: "회원 ID", value: <span className="font-mono text-xs">{seller.userId}</span> },
          ...
          <div>
            ...
            <KeyValueList
              items={[
                ...
                { label: "계좌번호", value: <span className="tabular-nums">{seller.bankAccount}</span> },
                { label: "예금주", value: seller.bankHolder },
              ]}
            />
            <p className="mt-2 text-xs text-zinc-500">
              구매자는 주문 후 이 계좌로 직접 송금합니다. 플랫폼은 판매대금을 보관하지 않습니다.
            </p>
          </div>
          <div>
            ...
            <KeyValueList
              items={[
                {
                  label: "사업자등록증",
                  value: <DocumentLink url={businessLicenseSignedUrl} label="사업자등록증 보기" />,
                },
                {
                  label: "통신판매업 신고증",
                  value: <DocumentLink url={mailOrderLicenseSignedUrl} label="통신판매업 신고증 보기" />,
                },
              ]}
            />
            <p className="mt-2 text-xs text-zinc-500">서류 링크는 일정 시간 후 만료됩니다. 만료 시 페이지를 새로고침하세요.</p>
          </div>
        </section>
      </div>

      <section className="mt-10">
        <SectionTitle right={<span className="text-sm text-zinc-500">최근 {products.length}건</span>}>등록 상품</SectionTitle>
        {products.length === 0 ? (
                ...
                {products.map((p) => (
                  <TableRow key={p.id}>
                    <TableCell className="font-medium">
                      <Link href={`/admin/phones/${p.id}`} className="hover:underline">
                        {productTitle(p)}
                      </Link>
                    </TableCell>
