...
export default async function MarketplaceDashboardPage() {
  ...
  const totalByStatus = ORDER_STATUSES.reduce(
    (a, s) => a + (stats.byStatus[s] ?? 0),
    0,
  );
  ...
  return (
    <AdminShell
      heading="중고폰 현황"
      description="중고폰 마켓플레이스 운영 지표 요약입니다."
    >
      ...
      <section className="mb-8 grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard
          label="입점 판매자 수"
          value={stats.approvedSellers.toLocaleString("ko-KR")}
          hint="승인 상태 판매자"
        />
        <StatCard
          label="판매중 상품 수"
          value={stats.onSaleProducts.toLocaleString("ko-KR")}
        />
        <StatCard
          label="오늘 상품 등록 수"
          value={stats.productsToday.toLocaleString("ko-KR")}
          hint="오늘 0시 이후 등록"
        />
        <StatCard
          label="주문 수"
        ...
        <StatCard
          label="입금완료 주문 수"
          value={stats.depositConfirmed.toLocaleString("ko-KR")}
          hint="판매자 입금확인 이후 단계"
        />
        <StatCard
          label="거래완료 수"
            ...
            <TableBody>
              {ORDER_STATUSES.map((s) => {
                ...
                return (
                  <TableRow
                    key={s}
                    className={n === 0 ? "text-zinc-400" : undefined}
                  >
                    ...
                    <TableCell className="text-right">
                      {n > 0 && (
                        <Link
                          href={`/admin/phone-orders?status=${s}`}
              ...
              <TableRow className="font-semibold">
                ...
                <TableCell className="text-right tabular-nums">
                  {totalByStatus.toLocaleString("ko-KR")}
                </TableCell>
                <TableCell></TableCell>
              </TableRow>
            </TableBody>
