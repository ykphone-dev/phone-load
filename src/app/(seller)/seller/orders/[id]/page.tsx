    ...
    <div className="flex flex-wrap gap-2">
      {urls.map((u) => (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="relative block h-20 w-20 overflow-hidden border bg-zinc-50"
        >
...
export default async function SellerOrderDetailPage({
  ...
  const activeRefund =
    order.refundRequests.find((r) =>
      ["REQUESTED", "REJECTED"].includes(r.status),
    ) ?? null;
  const openDispute =
    order.disputes.find((d) => d.status !== "RESOLVED") ?? null;
    ...
    <div className="max-w-4xl space-y-8">
      ...
      <section>
        ...
        <SellerOrderActions
          ...
          openDispute={
            openDispute
              ? { id: openDispute.id, sellerReply: openDispute.sellerReply }
              : null
          }
        />
      </section>
        ...
        <KeyValueList
          items={[
            ...
            { label: "주문일시", value: formatDateTime(order.createdAt) },
            {
              label: "금액",
              value: (
                <span className="font-semibold">{formatKRW(order.price)}</span>
              ),
            },
            {
              label: "입금기한",
      ...
      <section>
        ...
        <KeyValueList
          items={[
            {
              label: "구매자 입금표시",
              value: formatDateTime(order.transferReport?.buyerReportedAt),
            },
            {
              label: "입금자명",
              value: order.transferReport?.depositorName ?? "-",
            },
            {
              label: "판매자 확인",
            ...
            {
              label: "입금 계좌",
              value: `${order.seller.bankName} ${order.seller.bankAccount} (${order.seller.bankHolder})`,
            },
          ]}
        />
      </section>
              ...
              <div key={r.id} className="space-y-2 border p-3 text-sm">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className="font-medium">
                    {REFUND_REASON_LABEL[r.reason as RefundReason] ?? r.reason}
                  </span>
                ...
                <KeyValueList
                  ...
                  items={[
                    ...
                    ...(r.sellerNote
                      ? [{ label: "판매자 메모", value: r.sellerNote }]
                      : []),
                  ]}
                />
              </div>
                      ...
                      value: d.sellerReply ? (
                        <span className="whitespace-pre-wrap">
                          ...
                          <span className="ml-1 text-xs text-zinc-400">
                            ({formatDateTime(d.sellerRepliedAt)})
                          </span>
                        </span>
                      ) : (
                        "-"
                      ...
                      ? [
                          {
                            label: "처리 결과",
                            value: (
                              <OrderStatusBadge status={d.resolutionStatus} />
                            ),
                          },
                        ]
                      : []),
