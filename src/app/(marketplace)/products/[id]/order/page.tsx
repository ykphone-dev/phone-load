...
export default async function OrderPage({ params }: { params: { id: string } }) {
  const [product, ctx] = await Promise.all([getPublicProduct(params.id), getSessionContext()]);
  if (!product) notFound();
    ...
    <div className="mx-auto grid max-w-4xl gap-8 md:grid-cols-[1fr_320px]">
      <div>
        ...
        <OrderForm
          ...
          defaultName={(ctx.user?.user_metadata?.name as string | undefined) ?? ""}
        />
      </div>
      ...
      <aside className="h-fit space-y-4 border p-4 md:sticky md:top-20">
        ...
        <dl className="space-y-1 border-t pt-3 text-sm">
          <div className="flex justify-between"><dt className="text-zinc-500">판매자</dt><dd>{product.seller.businessName}</dd></div>
          <div className="flex justify-between text-base font-semibold"><dt>결제 금액</dt><dd>{formatKRW(product.price)}</dd></div>
        </dl>
        <p className="text-xs text-zinc-500">결제수단: 판매자 계좌 직접 이체</p>
        <Link href={`/products/${product.id}`} className="block text-center text-xs text-zinc-500 underline">상품으로 돌아가기</Link>
      </aside>
    </div>
  );
}
