...
export default async function BuyerOrderPage({ params }: { params: { orderNumber: string } }) {
  ...
  const waiting = order.status === "WAITING_DEPOSIT" || order.status === "DEPOSIT_REPORTED";
  const activeRefund = order.refundRequests.find((r) => r.status !== "WITHDRAWN" && r.status !== "CONFIRMED");
  const sellerRefunded = order.refundRequests.some((r) => r.status === "SELLER_REFUNDED");
      ...
      <section>
        ...
        <div className="flex gap-4 border p-4">
          ...
          <div className="space-y-1 text-sm">
            <Link href={`/products/${order.product.id}`} className="font-medium hover:underline">
              {productTitle(order.product)} · {order.product.color}
            </Link>
            <p className="text-zinc-500">{order.product.conditionGrade}등급 · {order.seller.businessName}</p>
      ...
      <section>
        ...
        <KeyValueList
          items={[
            ...
            { label: "구매자", value: `${order.buyerName} / ${order.buyerPhone}` },
            {
              label: "배송지",
              value: [order.shippingPostalCode, order.shippingAddress, order.shippingAddressDetail].filter(Boolean).join(" "),
            },
            ...
            { label: "판매자 입금확인", value: formatDateTime(order.transferReport?.sellerConfirmedAt) },
        ...
        <section>
          ...
          <KeyValueList
            items={[
              ...
              { label: "송장번호", value: <span className="font-mono">{order.shipment.trackingNumber}</span> },
              { label: "발송일", value: formatDateTime(order.shipment.shippedAt) },
              { label: "배송완료", value: formatDateTime(order.shipment.deliveredAt) },
            ]}
          />
          <p className="mt-2 text-xs text-zinc-500">송장번호로 택배사 홈페이지에서 배송을 조회할 수 있습니다.</p>
        </section>
      )}

      {activeRefund && (
        <section>
          <SectionTitle right={<RefundStatusBadge status={activeRefund.status} />}>환불 요청</SectionTitle>
          <KeyValueList
            items={[
              { label: "사유", value: REFUND_REASON_LABEL[activeRefund.reason as RefundReason] },
              { label: "내용", value: activeRefund.description || "-" },
              { label: "환불 계좌", value: `${activeRefund.refundBankName} ${activeRefund.refundAccount} (${activeRefund.refundHolder})` },
              { label: "요청일시", value: formatDateTime(activeRefund.requestedAt) },
              { label: "판매자 메모", value: activeRefund.sellerNote || "-" },
              { label: "판매자 환불처리", value: formatDateTime(activeRefund.refundedAt) },
            ]}
          />
        </section>
      )}
          ...
          <div className="space-y-3">
            {order.disputes.map((d) => (
              <div key={d.id} className="space-y-2 border p-4 text-sm">
                ...
                {d.description && <p className="whitespace-pre-wrap text-zinc-600">{d.description}</p>}
                <p className="text-xs text-zinc-500">접수 {formatDateTime(d.createdAt)}</p>
                {d.sellerReply && (
                ...
                {d.status === "RESOLVED" && (
                  <div className="border-l-2 border-emerald-500 pl-3">
                    <p className="text-xs text-zinc-500">관리자 처리 결과 ({formatDateTime(d.resolvedAt)})</p>
