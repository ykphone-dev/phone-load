...
const inspection = Object.fromEntries(
  INSPECTION_ITEMS.map((k) => [k, "NORMAL"]),
);
...
const baseProduct = {
  ...
  images: [
    {
      imageUrl:
        "https://x.supabase.co/storage/v1/object/public/phone-images/a.webp",
      kind: "FRONT",
    },
  ],
  publish: true,
...
describe("productFormSchema", () => {
  ...
  test("Apple 은 배터리 성능 % 필수", () => {
    ...
    if (r.success === false)
      expect(r.error.issues[0].path).toEqual(["batteryHealth"]);
  });
  ...
  test("Samsung 은 배터리 상태 필수", () => {
    ...
    expect(r.success).toBe(false);
    if (r.success === false)
      expect(r.error.issues[0].path).toEqual(["batteryStatus"]);
  });

  test("판매 등록 시 분실·도난 확인 체크와 사진 1장 이상 필수, 임시저장은 예외", () => {
    expect(
      productFormSchema.safeParse({ ...baseProduct, notLostOrStolen: false })
        .success,
    ).toBe(false);
    expect(
      productFormSchema.safeParse({ ...baseProduct, images: [] }).success,
    ).toBe(false);
    expect(
  ...
  test("수리이력 '있음' 이면 내용 필수", () => {
    expect(
      productFormSchema.safeParse({ ...baseProduct, repairHistory: "YES" })
        .success,
    ).toBe(false);
    expect(
      productFormSchema.safeParse({
...
describe("createOrderSchema", () => {
  ...
  test("판매계약 당사자 고지 미동의 시 실패", () => {
    expect(
      createOrderSchema.safeParse({ ...base, agreeNotParty: false }).success,
    ).toBe(false);
  });
  test("휴대폰 형식", () => {
    expect(
      createOrderSchema.safeParse({ ...base, buyerPhone: "02-123-4567" })
        .success,
    ).toBe(false);
  });
});
...
describe("sellerRegisterSchema", () => {
  test("사업자등록번호/계좌 형식", () => {
    ...
    expect(
      sellerRegisterSchema.safeParse({ ...base, businessNumber: "12345" })
        .success,
    ).toBe(false);
    expect(
      sellerRegisterSchema.safeParse({ ...base, bankAccount: "abc" }).success,
    ).toBe(false);
  });
});
