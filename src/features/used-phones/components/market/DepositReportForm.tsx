...
export function DepositReportForm({
  orderNumber,
  defaultName,
}: {
  orderNumber: string;
  ...
  const submit = () => {
    ...
    if (
      !window.confirm(
        "판매자 계좌로 입금을 완료하셨습니까?\n판매자가 실제 입금을 확인한 후 배송이 진행됩니다.",
      )
    )
      return;
    startTransition(async () => {
      const res = await reportDepositAction(orderNumber, {
      ...
      toast({
        title: "입금완료를 표시했습니다",
        description: "판매자가 입금을 확인하면 배송이 시작됩니다.",
      });
    ...
    <div className="space-y-3 border-t pt-4">
      <div className="space-y-1.5">
        ...
        <p className="text-xs text-zinc-500">
          주문자명과 입금자명이 다르면 판매자가 확인하기 어렵습니다. 실제 입금한
          이름을 적어주세요.
        </p>
      </div>
