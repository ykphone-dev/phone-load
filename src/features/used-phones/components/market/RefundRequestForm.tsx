...
import { refundRequestSchema, type RefundRequestInput } from "../../validations";
import { ImageUploader } from "../ImageUploader";

const selectClass = "flex h-10 w-full border bg-white px-3 text-sm";

/** 환불 요청 (기획서 21). 판매자가 소비자 계좌로 직접 환불하므로 환불 계좌를 받는다. */
export function RefundRequestForm({ orderNumber, buyerName }: { orderNumber: string; buyerName: string }) {
  ...
  if (!open) {
    return (
      <Button variant="outline" onClick={() => setOpen(true)}>환불 요청</Button>
    );
  }

  const onSubmit = (values: RefundRequestInput) => {
    if (!window.confirm("환불을 요청하시겠습니까? 판매자가 확인 후 환불 계좌로 송금합니다.")) return;
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 border p-4">
        ...
        <FormField control={form.control} name="description" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Textarea {...field} rows={3} placeholder="어떤 문제가 있는지 구체적으로 적어주세요" /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        <FormField control={form.control} name="imageUrls" render={({ field }) => (
          <FormItem>
            ...
            <FormControl>
              <ImageUploader scope="evidence" orderNumber={orderNumber} value={field.value} onChange={field.onChange} max={10} />
            </FormControl>
            <FormMessage />
          </FormItem>
        )} />
        <div className="grid gap-3 sm:grid-cols-3">
          <FormField control={form.control} name="refundBankName" render={({ field }) => (
            <FormItem>
          ...
          <FormField control={form.control} name="refundAccount" render={({ field }) => (
            <FormItem>
          ...
          <FormField control={form.control} name="refundHolder" render={({ field }) => (
            <FormItem>
        ...
        <FormDescription>환불은 판매자가 위 계좌로 직접 송금합니다. 환불금을 받으면 &quot;환불금 확인&quot; 버튼을 눌러주세요.</FormDescription>
