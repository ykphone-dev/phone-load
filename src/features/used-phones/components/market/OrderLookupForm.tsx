...
export function OrderLookupForm({ defaultOrderNumber = "" }: { defaultOrderNumber?: string }) {
  ...
  const form = useForm<OrderLookupInput>({
    ...
    defaultValues: { orderNumber: defaultOrderNumber, buyerPhone: "", password: "" },
  });
  ...
  const onSubmit = (values: OrderLookupInput) => {
    startTransition(async () => {
      ...
      if (res.ok === false) {
        toast({ title: "조회 실패", description: res.error, variant: "destructive" });
        return;
      }
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <FormField control={form.control} name="orderNumber" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Input {...field} placeholder="ORDER-20260909-000123" className="font-mono" /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        <FormField control={form.control} name="buyerPhone" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Input {...field} placeholder="010-0000-0000" inputMode="tel" /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
