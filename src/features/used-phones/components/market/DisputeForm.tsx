...
export function DisputeForm({ orderNumber }: { orderNumber: string }) {
  ...
  if (!open) {
    return (
      <Button variant="destructive" onClick={() => setOpen(true)}>문제가 해결되지 않았습니다</Button>
    );
  }

  const onSubmit = (values: DisputeOpenInput) => {
    if (!window.confirm("분쟁을 접수하시겠습니까? 옆커폰 관리자가 확인 후 처리합니다.")) return;
    startTransition(async () => {
      const res = await openDisputeAction(orderNumber, values);
      if (res.ok === false) {
        toast({ title: "접수 실패", description: res.error, variant: "destructive" });
        return;
      }
      toast({ title: "분쟁이 접수되었습니다", description: "관리자가 판매자 답변을 받아 처리합니다." });
      setOpen(false);
      router.refresh();
    });
  };
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 border border-rose-200 p-4">
        <h3 className="font-semibold text-rose-700">분쟁 접수</h3>
        <FormField control={form.control} name="reason" render={({ field }) => (
          <FormItem>
            <FormLabel>사유 (한 줄 요약)</FormLabel>
            <FormControl><Input {...field} placeholder="예: 환불 요청 후 3일간 판매자 응답 없음" /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        ...
        <FormDescription>접수 후에는 관리자가 처리할 때까지 주문 상태가 &quot;분쟁중&quot; 으로 유지됩니다.</FormDescription>
