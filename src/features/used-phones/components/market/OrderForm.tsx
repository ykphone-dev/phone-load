...
export function OrderForm({ productId, isLoggedIn, defaultName }: Props) {
  ...
  const onSubmit = (values: CreateOrderInput) => {
    if (!isLoggedIn && !values.password) {
      form.setError("password", { message: "비회원 주문은 주문조회 비밀번호가 필요합니다" });
      return;
    }
    startTransition(async () => {
      const res = await createOrderAction(values);
      if (res.ok === false) {
        toast({ title: "주문 실패", description: res.error, variant: "destructive" });
        if (res.status === 409) router.refresh();
        return;
      }
      toast({ title: "주문이 생성되었습니다", description: `주문번호 ${res.data.orderNumber}` });
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        ...
        <FormField control={form.control} name="buyerPhone" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Input {...field} placeholder="010-0000-0000" inputMode="tel" autoComplete="tel" /></FormControl>
            <FormDescription>주문조회와 배송 연락에 사용됩니다.</FormDescription>
            <FormMessage />
          </FormItem>
        )} />
        ...
        <FormField control={form.control} name="shippingAddressDetail" render={({ field }) => (
          <FormItem>
        ...
        <FormField control={form.control} name="shippingMemo" render={({ field }) => (
          <FormItem>
            <FormLabel>배송메모</FormLabel>
            <FormControl><Textarea {...field} rows={2} placeholder="부재 시 문 앞에 놓아주세요" /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        ...
        {!isLoggedIn && (
          <FormField control={form.control} name="password" render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>비회원 주문입니다. 주문번호·휴대폰번호·이 비밀번호로 주문을 조회할 수 있습니다.</FormDescription>
              <FormMessage />
            </FormItem>
          )} />
        )}

        <FormField control={form.control} name="agreeNotParty" render={({ field }) => (
          <FormItem className="rounded-sm border bg-zinc-50 p-3">
            <div className="flex items-start gap-3">
              <FormControl>
                <Checkbox checked={field.value === true} onCheckedChange={(v) => field.onChange(v === true ? true : undefined)} />
              </FormControl>
              <div className="space-y-1 text-sm">
                <FormLabel className="font-medium">판매계약 당사자 고지에 동의합니다</FormLabel>
                <p className="text-xs text-zinc-600">{PLATFORM_NOTICE_NOT_PARTY}</p>
              </div>
            </div>
            <FormMessage />
          </FormItem>
