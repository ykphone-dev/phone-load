...
import { BATTERY_STATUS_LABEL, BRAND_LABEL, type BatteryStatus, type Brand } from "../constants";
    ...
    <Link href={href ?? `/products/${product.id}`} className="group block">
      <div className="relative aspect-square w-full overflow-hidden bg-zinc-100">
        {image ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-xs text-zinc-400">사진 없음</div>
        )}
        {unavailable && (
        ...
        <GradeBadge grade={product.conditionGrade} className="absolute left-2 top-2" />
      </div>
      <div className="mt-2 space-y-0.5">
        <p className="text-xs text-zinc-500">{BRAND_LABEL[product.brand as Brand] ?? product.brand}</p>
        ...
        {product.seller && <p className="text-xs text-zinc-500">{product.seller.businessName}</p>}
      </div>
    </Link>
  );
}
