...
export const EMPTY_INSPECTION: ProductFormInput["inspection"] = Object.fromEntries(
...
export function ProductForm({ mode, productId, defaultValues }: Props) {
  ...
  const missingKinds = REQUIRED_IMAGE_KINDS.filter((k) => !images.some((img) => img.kind === k));
  ...
  const onSubmit = (values: ProductFormInput) => {
    startTransition(async () => {
      ...
      toast({
        ...
        description: values.publish ? "소비자에게 바로 노출됩니다." : "상품관리에서 언제든 판매 등록할 수 있습니다.",
      });
  ...
  return (
    <Form {...form}>
      <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <EnumSelect value={field.value} onChange={field.onChange} options={BRANDS} labels={BRAND_LABEL} />
                  <FormMessage />
                </FormItem>
              )}
            />
        ...
        {/* ② 상태 */}
        <Section no={2} title="상태" desc="표준화된 항목으로 정직하게 입력해주세요. 분쟁 시 기준이 됩니다.">
          <FormField
            control={form.control}
            name="conditionGrade"
            render={({ field }) => (
                  ...
                  {CONDITION_GRADES.map((g) => (
                    <button
                      ...
                      className={cn(
                        ...
                        conditionGrade === g ? "border-black bg-black text-white" : "hover:bg-zinc-50",
                      )}
                    >
                      <span className="block text-base font-bold">{g}</span>
                      <span className={cn("block text-xs", conditionGrade === g ? "text-zinc-200" : "text-zinc-500")}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <EnumSelect value={field.value} onChange={field.onChange} options={SCREEN_CONDITIONS} labels={SCREEN_CONDITION_LABEL} />
                  <FormMessage />
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <EnumSelect value={field.value} onChange={field.onChange} options={FRAME_CONDITIONS} labels={FRAME_CONDITION_LABEL} />
                  <FormMessage />
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <EnumSelect value={field.value} onChange={field.onChange} options={BACK_CONDITIONS} labels={BACK_CONDITION_LABEL} />
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="sm:max-w-xs">
                  ...
                  <FormDescription>설정 &gt; 배터리 &gt; 배터리 성능 상태의 최대 성능 수치</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="sm:max-w-xs">
                  ...
                  <EnumSelect value={field.value} onChange={field.onChange} options={BATTERY_STATUSES} labels={BATTERY_STATUS_LABEL} />
                  <FormDescription>삼성 멤버스 &gt; 진단 결과 기준</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <EnumSelect value={field.value} onChange={field.onChange} options={HISTORY_STATES} labels={HISTORY_STATE_LABEL} />
                    <FormMessage />
                  </FormItem>
                )}
              />
              {repairHistory === "YES" && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormControl>
                        <Textarea rows={2} placeholder="수리 내용 (예: 2024.03 액정 교체, 공식 서비스센터)" {...field} value={field.value ?? ""} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              {partsReplacement === "YES" && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormControl>
                        <Textarea rows={2} placeholder="교체 부품 (예: 배터리 교체, 비정품)" {...field} value={field.value ?? ""} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <FormItem className="rounded-md border bg-zinc-50 p-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    <FormLabel className="font-medium">분실·도난 제품이 아닙니다</FormLabel>
                    <FormDescription>판매 등록을 위해 반드시 확인해야 합니다. 허위 확인 시 모든 책임은 판매자에게 있습니다.</FormDescription>
                  </div>
                </div>
                <FormMessage />
              </FormItem>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <EnumSelect value={field.value} onChange={field.onChange} options={AVAILABILITY_STATES} labels={AVAILABILITY_LABEL} />
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="숫자 15자리" inputMode="numeric" maxLength={16} {...field} value={field.value ?? ""} />
                  </FormControl>
                  <FormDescription>소비자에게는 전체 IMEI가 노출되지 않습니다.</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
        ...
        {/* ③ 기능검사 */}
        <Section no={3} title="기능검사" desc="확인하지 않은 항목은 '미확인'으로 두세요.">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between gap-2 border px-3 py-2">
                    <FormLabel className="text-sm">{INSPECTION_ITEM_LABEL[item]}</FormLabel>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="sm:max-w-xs">
                ...
                <FormDescription>
                  {price ? <span className="font-medium text-black">{formatKRW(price)}</span> : "원 단위로 입력하세요."}
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
        ...
        <Section no={5} title="사진">
          <FormField
            ...
            name="images"
            render={({ field }) => {
              const list = (field.value ?? []) as { imageUrl: string; kind: ImageKind }[];
              return (
                <FormItem>
                  <FormDescription>필수: 전면·후면·좌측·우측·액정 5장 권장, 최대 {MAX_PRODUCT_IMAGES}장</FormDescription>
                  <FormControl>
                    <ImageUploader
                      scope="product"
                      ...
                      onChange={(urls) =>
                        field.onChange(
                          urls.map((url) => list.find((i) => i.imageUrl === url) ?? { imageUrl: url, kind: "ETC" as ImageKind }),
                        )
                      }
                    />
                  </FormControl>
                  {list.length > 0 && (
                    <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                      {list.map((img, i) => (
                        <div key={img.imageUrl + i} className="flex items-center gap-2 border p-2">
                          <div className="relative h-10 w-10 shrink-0 overflow-hidden bg-zinc-50">
                            <Image src={img.imageUrl} alt="" fill sizes="40px" className="object-cover" />
                          </div>
                          <span className="w-10 shrink-0 text-xs text-zinc-500">사진 {i + 1}</span>
                          <Select
                            value={img.kind}
                            onValueChange={(v) =>
                              field.onChange(list.map((it, j) => (j === i ? { ...it, kind: v as ImageKind } : it)))
                            }
                          >
                  ...
                  {missingKinds.length > 0 && (
                    <p className="mt-2 text-xs text-amber-700">
                      아직 없는 권장 사진: {missingKinds.map((k) => IMAGE_KIND_LABEL[k]).join(", ")}
                    </p>
                  )}
                  <FormMessage />
                </FormItem>
